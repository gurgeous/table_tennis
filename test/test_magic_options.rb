module TableTennis
  class TestMagicOptions < Minitest::Test
    ...
    def test_types
      ...
      # proc custom error
      assert_raises(ArgumentError, "ugh") { magic(proc_err: "nope") }
    end

    def test_aliases
      m = MagicOptions.new({a: :bool})
      assert_equal :bool, m.magic_resolve(:boolean)
      assert_equal :ints, m.magic_resolve(:integers)
      assert_equal Numeric, m.magic_resolve(:num)
    end

    def test_coercion
      m = MagicOptions.new({})
      assert_equal true, m.magic_coerce(1, :bool)
      assert_equal false, m.magic_coerce("false", :bool)
    end
